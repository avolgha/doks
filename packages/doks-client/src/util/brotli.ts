...
function initState(s: State): void {
  ...
  s.blockTrees[0] = 7;
  s.distRbIdx = 3;
  const maxDistanceAlphabetLimit: number = calculateDistanceAlphabetLimit(
    0x7ffffffc,
    3,
    120
  );
...
function readMetablockHuffmanCodesAndContextMaps(s: State): void {
  ...
  const numLiteralTrees: number = decodeContextMap(
    contextMapLength,
    s.contextMap,
    s
  );
  ...
  const numDistTrees: number = decodeContextMap(
...
function initializeCompoundDictionaryCopy(
  s: State,
  address: number,
  length: number
): void {
...
function copyFromCompoundDictionary(s: State, fence: number): number {
  ...
  while (s.cdBrLength !== s.cdBrCopied) {
    ...
    s.ringBuffer.set(
      s.cdChunks[s.cdBrIndex].slice(s.cdBrOffset, s.cdBrOffset + length),
      pos
    );
    pos += length;
    s.cdBrOffset += length;
    ...
    switch (s.runningState) {
      ...
      case 7:
        if (s.trivialLiteralContext !== 0) {
          ...
          let prevByte1: number =
            ringBuffer[(s.pos - 1) & ringBufferMask] & 0xff;
          let prevByte2: number =
